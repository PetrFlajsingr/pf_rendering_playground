...
namespace libassert::config {
    static std::atomic_bool output_colors = false;
...
namespace libassert::detail {
            ...
            if(SymFromAddr(proc, (DWORD64)addrs[i], &displacement.a, symbol)) {
                if(got_line) {
                    ...
                    if(SymSetContext(proc, &frame, nullptr) == FALSE && GetLastError() != ERROR_SUCCESS) {
                        fprintf(stdout, "Stack trace: Internal error while calling SymSetContext\n");
    ...
    public:
        ...
        LIBASSERT_ATTR_COLD bool pseudoparse(
            ...
            #endif
            if(depth > max_depth) {
                fprintf(stdout, "Max depth exceeded\n");
                return false;
            }
